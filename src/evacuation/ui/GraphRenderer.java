...
/**
 * Composant Swing qui dessine le graphe, les agents et les statistiques de densité.
 * Supporte la sélection d'un agent par clic souris pour visualiser son trajet restant.
 */
public class GraphRenderer extends JPanel {
    ...
    /** Callback appelé quand l'utilisateur sélectionne un agent. */
    ...
    private void dessinerAretes(Graphics2D g) {
        ...
        for (Edge edge : engine.getGraph().getEdges()) {
            ...
            int x1 = src.getX()  * SCALE + OFFSET;
            int y1 = src.getY()  * SCALE + OFFSET;
            int x2 = dest.getX() * SCALE + OFFSET;
            int y2 = dest.getY() * SCALE + OFFSET;
            ...
            if (edge.isDisponible()) {
                ...
                g.drawString(String.valueOf((int) edge.getPoids()),
                        (x1 + x2) / 2 + 4, (y1 + y2) / 2 - 4);
            }
        }
    }
    ...
    /**
     * Surligne en cyan le trajet restant de l'agent sélectionné.
     */
    ...
    private void dessinerNoeuds(Graphics2D g, int[] densite) {
        ...
        for (int i = 0; i < graph.getNodes().size(); i++) {
            ...
            int x = node.getX() * SCALE + OFFSET;
            int y = node.getY() * SCALE + OFFSET;

            // Couleur de base selon le type
            Color couleurBase;
            if (node.isBloque()) {
                couleurBase = Color.BLACK;
            } else if (node instanceof ZoneDanger) {
                couleurBase = Color.ORANGE;
            } else if (node instanceof Sortie) {
                couleurBase = ((Sortie) node).isEstOuverte() ? Color.GREEN : Color.LIGHT_GRAY;
            } else {
                couleurBase = new Color(100, 149, 237);
            }

            // Superpose un dégradé rouge selon la densité
            ...
            // Bordure plus épaisse si nœud de l'agent sélectionné
            boolean estPositionAgent = agentSelectionne != null
                    && agentSelectionne.getPosition().equals(node);
            g.setColor(estPositionAgent ? new Color(0, 200, 255) : Color.BLACK);
            g.setStroke(new BasicStroke(estPositionAgent ? 3f : 1.5f));
            ...
            if (densite[i] > 0) {
                ...
                g.drawString("×" + densite[i], x + NODE_RADIUS - 4, y - NODE_RADIUS + 10);
            }
        }
    }

    /** Mélange deux couleurs selon un ratio [0-1]. */
    ...
    private void dessinerAgents(Graphics2D g) {
        ...
        for (Agent agent : agents) {
            ...
            int x = pos.getX() * SCALE + OFFSET;
            int y = pos.getY() * SCALE + OFFSET;

            // Décalage léger si plusieurs agents au même nœud
            ...
            // Cercle plus grand si agent sélectionné
            boolean selectionne = agent.equals(agentSelectionne);
            int r = selectionne ? 11 : 8;

            g.setColor(couleur);
            g.fillOval(x + dx - r, y + dy - r, r * 2, r * 2);
            g.setColor(selectionne ? new Color(0, 200, 255) : Color.BLACK);
            g.setStroke(new BasicStroke(selectionne ? 2.5f : 1.5f));
    ...
    private void dessinerLegende(Graphics2D g) {
        ...
        g.setFont(new Font("Arial", Font.BOLD, 12));
        g.setColor(Color.BLACK);
        g.drawString("Légende :", x, y);

        dessinerEntree(g, x, y + 18,  new Color(100, 149, 237), "Nœud normal");
        dessinerEntree(g, x, y + 36,  Color.GREEN,               "Sortie ouverte");
        dessinerEntree(g, x, y + 54,  Color.LIGHT_GRAY,          "Sortie fermée");
        dessinerEntree(g, x, y + 72,  Color.ORANGE,              "Zone de danger / Agent panique");
        dessinerEntree(g, x, y + 90,  Color.BLACK,               "Nœud bloqué");
        dessinerEntree(g, x, y + 108, Color.YELLOW,              "Agent en mouvement");
        dessinerEntree(g, x, y + 126, Color.MAGENTA,             "Agent en folie");
        dessinerEntree(g, x, y + 144, new Color(0, 200, 255),    "Trajet sélectionné");
    }

    private void dessinerEntree(Graphics2D g, int x, int y, Color couleur, String label) {
        g.setColor(couleur);
        g.fillRect(x, y - 10, 14, 14);
        g.setColor(Color.BLACK);
        g.drawRect(x, y - 10, 14, 14);
        g.setFont(new Font("Arial", Font.PLAIN, 11));
    ...
    /** Cherche l'agent le plus proche du clic (rayon de détection 15px). */
    private Agent trouverAgentAuClic(int mx, int my) {
        for (Agent agent : engine.getAgents()) {
            if (agent.getEtatDeplacement() == Agent.EtatDeplacement.ARRIVE) continue;
            Node pos = agent.getPosition();
            int ax = pos.getX() * SCALE + OFFSET;
            int ay = pos.getY() * SCALE + OFFSET;
